...
raw_datasets = load_dataset(
    "json",
    data_files={
        "train": "en_pt_es_data/train_en_pt.json",
        "valid": "en_pt_es_data/valid_en_pt.json",
    },
    field="data",
)
...
(
    ...
    en_tokenizer.cls_token,
    en_tokenizer.sep_token,
    en_tokenizer.pad_token,
    en_tokenizer.mask_token,
) = ["[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]"]
(
    es_tokenizer.unk_token,
    es_tokenizer.cls_token,
    es_tokenizer.sep_token,
    es_tokenizer.pad_token,
    es_tokenizer.mask_token,
) = ["[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]"]
...
def process_data_to_model_inputs(batch):
    ...
    inputs = en_tokenizer(
        inputs, padding="max_length", truncation=True, max_length=encoder_max_length
    )
    outputs = es_tokenizer(
        targets, padding="max_length", truncation=True, max_length=decoder_max_length
    )
    ...
    batch["labels"] = [
        [-100 if token == en_tokenizer.pad_token_id else token for token in labels]
...
def compute_metrics(eval_preds):
    ...
    prediction_lens = [
        np.count_nonzero(pred != es_tokenizer.pad_token_id) for pred in pred_ids
    ]
